...
void ssd1306_circle(ssd1306_t *ssd, int x0, int y0, int radius, bool value, bool fill) {
  int x = 0;
  int y = radius;
  int d = 3 - 2 * radius;
  ...
  while (x <= y) {
      if (fill) {
          // Desenha linhas horizontais entre os pontos simétricos
          ssd1306_hline(ssd, x0 - x, x0 + x, y0 + y, value);
          ssd1306_hline(ssd, x0 - x, x0 + x, y0 - y, value);
          ssd1306_hline(ssd, x0 - y, x0 + y, y0 + x, value);
          ssd1306_hline(ssd, x0 - y, x0 + y, y0 - x, value);
      } else {
          // Desenha apenas os pontos da borda
